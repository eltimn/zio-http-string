      ...
      case Method.GET -> !! => 
        Response.html(
          withContentHtml(h2(styleAttr := List("border" -> "none"), "You're running on ZIO Http!")) {
            div(
              p("""This starter project may help you getting started with your own web application, zio-http-string:0.0.1 """),
          ...
          withContentHtml(h2("Hot-Reload changes with SBT-Revolver")) {
            div(
              ul(
                ...
                li(pre("sbt:zio-http-string> ~reStart")),
              ), 
          ...
          withContentHtml(h2("Dockerize this application")) {
            div(
              ul(
                ...
                li(pre("sbt:zio-http-string> Docker / publishLocal")), 
                li(h4("Then run a container")), 
                li(pre("$ docker run -p 8080:8080 zio-http-string:0.0.1")),
